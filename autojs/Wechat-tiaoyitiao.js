class mainFn {
  constructor(window, windowid, ar) {
...
var sf = 1.37;
dialogs.alert("touch start and end");
...
var window_ = floaty.window(
  <frame>
    <img
      w="{{Math.floor(device.width*0.14)}}px"
      h="{{Math.floor(device.width*0.14)}}px"
      src="#88eeeeee"
      radius="{{Math.floor(device.width*0.7)}}px"
    />
    <img
      id="but_"
      w="{{Math.floor(device.width*0.14)}}px"
      h="{{Math.floor(device.width*0.14)}}px"
      src="@drawable/ic_details_black_48dp"
    />
  </frame>
);
var rad = new mainFn(Rwindow, Rwindow.but, 1);
var rd = new mainFn(window_, window_.but_);
...
var Tx, Ty, TX, TY;
Rwindow.img.setOnTouchListener(
  new android.view.View.OnTouchListener((view, event) => {
    switch (event.getAction()) {
      case event.ACTION_DOWN:
        Tx = event.getRawX();
...
/*
var mainFn = function (window, windowid, ar) {
  this.Orientation = context.resources.configuration.orientation;
  this.Width = this.Orientation == 1 ? device.width : device.height;
  this.Height = this.Orientation == 2 ? device.width : device.height;
  this.Click = function () {};
